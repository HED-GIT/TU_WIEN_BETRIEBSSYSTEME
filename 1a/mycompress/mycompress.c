...
#define USAGE()                                                    		\
	do                                                             		\
	{                                                              		\
		fprintf(stdout, "USAGE: %s [-o outfile] [file...]\n", name); 	\
		exit(EXIT_FAILURE);                                        		\
	} while (0)
 ...
 */
static void handle_files(FILE *outFile, FILE *inFile)
{
	int next;
	int current = EOF;
	int amount = 0;
	...
	while (1)
	{
		next = fgetc(inFile);
		if (current == EOF)
		{ // current can only equal EOF at start of program, used to init current
			current = next;
			if (next == EOF)
			{ //without this it would lead to an infinity loop if eof was returned on first iteration
				if (!feof(inFile))
				{
		...
		if (next != current)
		{
			...
			{
				if (!feof(inFile))
				{ // checks if EOF was reached or another error occured
					ERROR_MSG("error while reading file, EOF not reached");
				}
				break;
			}
...
int main(int argc, char **argv)
{
	...
	if (outFile == NULL)
	{ // this is done here because only the last fopen is important, else the program would throw an error on a file that isn't used anyway
		ERROR_EXIT("couldn't open output file");
	}

	if (optind == argc)
	{
		handle_files(outFile, stdin);
	}
	else
	{
